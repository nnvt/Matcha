...
exports.completeInfos = async (req, res, next) => {
	...
	try {
		...
		// Kiểm tra xem thông tin người dùng đã hoàn thành chưa
		const isCompleted = await isUserProfileCompleted(user);

		if (!isCompleted) {
			return res
				.status(401)
				.json({ success: false, error: "Must complete your profile informations !" });
		}
		next();
	} catch (error) {
		return res
			...
			.json({
				success: false,
				error: "An error has occurred while validating account informations, try later!",
			});
	}
};
