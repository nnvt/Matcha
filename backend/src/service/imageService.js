...
const find = async (where) => {
  try {
    const result = await db.images.findAll({
...
const findOne = async (id, uid) => {
  try {
    const res = await db.images.findOne({
      where: {
        id: id, user_id: uid
      },
    });
...
const findUserImages = async (userid) => {
  try {
    const res = await db.images.findAll({
      include: [
        {
          model: db.users,
          as: 'user',
...
const findPicture = async (userid, url) => {
  try {
    const result = await db.images.findOne({
...
const save = async (image) => {
  try {
    const newImage = await db.images.create(image); // Create a new image record
    if (image.profile) {
